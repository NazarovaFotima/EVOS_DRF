...
class ProductViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.category2 = Category.objects.create(title='Books')

        self.product1 = Product.objects.create(title='Laptop', description='Powerful Laptop', category=self.category1, price=5000, cost=5000)
        self.product2 = Product.objects.create(title='Book', description='Thriller Book', category=self.category2, price=6000, cost=5000)
    ...
    def test_product_detail(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['products']['title'], 'Laptop')
    ...
    def test_permission_denied_for_anonymous_create(self):
        ...
        url = reverse('product-list')
        data = {'title': 'Test Product', 'description': 'This is a test product', 'price': 10.00, 'cost':10.00}
