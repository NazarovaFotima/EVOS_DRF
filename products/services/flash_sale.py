...
from products.models.misc import Product, ProductViewHistory

from rest_framework import generics, serializers
from products.models.flashsale import FlashSale
...
def check_flash_sale(request, product_id):
    ...
    # Check if the user has viewed this products before
    user_viewed = ProductViewHistory.objects.filter(user=request.user, product=product).exists()

    # Check if the products is or will be on a flash sale within the next 24 hours
    ...
    if user_viewed and upcoming_flash_sale:
        ...
        return Response({
            "message": f"This products will be on a {discount}% off flash sale!",
            "start_time": start_time,
            "end_time": end_time
        })
    else:
        return Response({
            "message": "No upcoming flash sales for this products."
        })